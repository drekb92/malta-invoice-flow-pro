 ...
 */
export const createCreditNoteFromInvoice = async (invoiceId: string, userId: string) => {
  ...
  if (headerError || !headerData?.id) {
    ...
    // surface unique-constraint issues with a friendly message
...
// Keep the old object-style service for any existing imports
