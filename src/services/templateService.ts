...
export const getDefaultTemplate = async (): Promise<InvoiceTemplate> => {
  try {
    ...
    return {
      ...
      header_layout: data.header_layout || 'default',
      table_style: data.table_style || 'default',
      totals_style: data.totals_style || 'default',
      banking_visibility: data.banking_visibility !== undefined ? data.banking_visibility : true,
      banking_style: data.banking_style || 'default',
